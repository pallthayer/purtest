	...
	public class LinePlot extends MovieClip{
		...
		public function loadData(value:Array, summary:MovieClip){
			...
			for(var i=0;i<value.length;i++){
				...
				dataPoint.addEventListener(MouseEvent.MOUSE_OVER, rollover );
				dataPoint.addEventListener(MouseEvent.MOUSE_OUT, rollout );
		...
		public function plot(type:String, value:Array, startDate:String, max:Object, mean:Object, plotHeight:int, gridHeight:int):void{
			...
			_piecewiseLinear.graphics.lineStyle(4, 0xFFE383, 1, false, LineScaleMode.VERTICAL, CapsStyle.NONE, JointStyle.MITER, 2);
			...
			_maxLine = new Shape();		
			_maxLine.graphics.lineStyle(3, 0x7E7872, 1, false, LineScaleMode.VERTICAL, CapsStyle.NONE, JointStyle.MITER, 2);
			_aveLine = new Shape();
			_aveLine.graphics.lineStyle(3, 0x7E7872, 1, false, LineScaleMode.VERTICAL, CapsStyle.NONE, JointStyle.MITER, 2);
			...
			//The following create a display box on the current week's node - commented out
			//_dataPoints[_dataPoints.length-1]._nodeBtn.dispatchEvent(new MouseEvent(MouseEvent.MOUSE_OVER));
		...
		public function rollover(event:MouseEvent){
			...
			if(event.target.parent.x < this.width - _infoBox.width){
				_infoBox.x = event.target.parent.x+35;
				_infoBox.y = event.target.parent.y+15;
			}
			else{
		...
		public function rollout(vent:MouseEvent){
			removeChild(_infoBox);
		}
		...
		public function drawLabels():void{
			...
			for(i=0; i < _dataPoints.length-1; i++){
